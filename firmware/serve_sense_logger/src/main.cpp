#include <Arduino.h>
#include <Wire.h>
#include <NimBLEDevice.h>

// ---------- I2C pins & ICM20600 config ----------
#define I2C_SDA 5
#define I2C_SCL 6
#define ICM20600_ADDR 0x69

#define PIN_CAPTURE_BTN 0  // BOOT button (active low)
#define PIN_RECORD_SWITCH D1  // D1 on XIAO ESP32S3 (switch: ON=LOW starts recording, with pullup)
#define PIN_STATUS_LED 21  // Built-in LED on XIAO ESP32S3

#define REG_PWR_MGMT_1   0x6B
#define REG_ACCEL_CONFIG 0x1C
#define REG_GYRO_CONFIG  0x1B
#define REG_ACCEL_XOUT_H 0x3B
#define REG_WHO_AM_I     0x75

// conversions for ±2g accel, ±250 dps gyro
static const float ACC_LSB_PER_G   = 16384.0f;
static const float GYR_LSB_PER_DPS = 131.0f;

// ---------- BLE UUIDs ----------
static const NimBLEUUID SVC_UUID ((uint16_t)0xFF00);
static const NimBLEUUID IMU_UUID ((uint16_t)0xFF01);  // notify
static const NimBLEUUID CTRL_UUID((uint16_t)0xFF02);  // write

// ---------- Globals ----------
NimBLECharacteristic* imuChar = nullptr;
volatile bool streamOn = true;
const uint32_t PERIOD_MS = 25;  // ~40 Hz

// ServeSense specific globals
volatile bool captureEnabled = false;
volatile bool serveMarker = false;
uint32_t sessionId = 0;
uint32_t sampleSerial = 0;

// ServeSense packet structure
struct ServePacket {
  uint32_t millis_ms;
  uint32_t session;
  uint32_t sequence;
  float ax, ay, az;
  float gx, gy, gz;
  uint8_t flags;
} __attribute__((packed));

// ---------- I2C helpers ----------
static bool i2cWrite(uint8_t reg, uint8_t val) {
  Wire.beginTransmission((uint8_t)ICM20600_ADDR);
  Wire.write(reg);
  Wire.write(val);
  return Wire.endTransmission() == 0;
}

static bool i2cReadBytes(uint8_t reg, uint8_t* buf, size_t len) {
  Wire.beginTransmission((uint8_t)ICM20600_ADDR);
  Wire.write(reg);
  if (Wire.endTransmission(false) != 0) return false;

  if (Wire.requestFrom((uint8_t)ICM20600_ADDR, (uint8_t)len) != (int)len) return false;
  for (size_t i = 0; i < len; ++i) {
    buf[i] = Wire.read();
  }
  return true;
}

static bool icmInit(uint8_t& who) {
  who = 0;
  if (!i2cWrite(REG_PWR_MGMT_1, 0x01)) return false;  // wake, PLL
  delay(50);
  if (!i2cWrite(REG_ACCEL_CONFIG, 0x00)) return false; // ±2g
  if (!i2cWrite(REG_GYRO_CONFIG,  0x00)) return false; // ±250 dps
  delay(10);
  return i2cReadBytes(REG_WHO_AM_I, &who, 1);
}

static bool icmRead(float& ax,float& ay,float& az,
                    float& gx,float& gy,float& gz) {
  uint8_t raw[14];
  if (!i2cReadBytes(REG_ACCEL_XOUT_H, raw, sizeof(raw))) return false;

  auto s16 = [&](int i)->int16_t {
    return (int16_t)((raw[i] << 8) | raw[i+1]);
  };

  ax = s16(0)  / ACC_LSB_PER_G;
  ay = s16(2)  / ACC_LSB_PER_G;
  az = s16(4)  / ACC_LSB_PER_G;

  gx = s16(8)  / GYR_LSB_PER_DPS;
  gy = s16(10) / GYR_LSB_PER_DPS;
  gz = s16(12) / GYR_LSB_PER_DPS;

  return true;
}

// ---------- BLE callbacks ----------
class CbServer : public NimBLEServerCallbacks {
  void onConnect(NimBLEServer* server) {
    Serial.print("[BLE] Central connected\r\n");
  }
  
  void onDisconnect(NimBLEServer*) {
    Serial.print("[BLE] Central disconnected\r\n");
    Serial.print("[BLE] Restarting advertising\r\n");
    NimBLEDevice::startAdvertising();
  }
};

class CbCtrl : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* c) {
    auto v = c->getValue();
    if (v.size() > 0) {
      streamOn = (v[0] != 0);
      Serial.print("[CTRL] streamOn = ");
      Serial.print(streamOn ? 1 : 0);
      Serial.print("\r\n");
    }
  }
};

void IRAM_ATTR onButtonFalling() {
  static uint32_t last = 0;
  uint32_t now = millis();
  if (now - last < 200) return; // debounce
  last = now;
  captureEnabled = !captureEnabled;
  if (captureEnabled) {
    sessionId++;
    sampleSerial = 0;
  }
  serveMarker = true;
}

// ============================================================
// Setup
// ============================================================

void setup() {
  // ESP32S3 XIAO USB CDC setup
  Serial.begin(115200);
  pinMode(PIN_CAPTURE_BTN, INPUT_PULLUP);
  pinMode(PIN_RECORD_SWITCH, INPUT_PULLUP); // Switch: LOW = ON (closed to GND/right), HIGH = OFF (open or VCC/left)
  pinMode(PIN_STATUS_LED, OUTPUT);
  digitalWrite(PIN_STATUS_LED, LOW); // Start with LED off



  attachInterrupt(digitalPinToInterrupt(PIN_CAPTURE_BTN), onButtonFalling, FALLING);

  // Wait for USB CDC to be ready on ESP32S3 in Windows environment
  while (!Serial && millis() < 5000) {
    delay(10);
  }
  delay(1000);
  Serial.flush();
  
  Serial.println("\n[BOOT] ServeSense Logger");

  // I2C + IMU
  Wire.begin(I2C_SDA, I2C_SCL, 400000);
  Serial.print("[I2C] Initializing ICM20600...\r\n");
  
  uint8_t who = 0;
  if (icmInit(who)) {
    Serial.print("[I2C] ICM20600 OK, WHO_AM_I=0x");
    Serial.print(who, HEX);
    Serial.print(" (addr 0x");
    Serial.print(ICM20600_ADDR, HEX);
    Serial.print(")\r\n");
  } else {
    Serial.print("[I2C] ICM init FAILED\r\n");
  }

  // BLE setup
  Serial.print("[BLE] Initializing NimBLE...\r\n");
  NimBLEDevice::init("ServeSense");
  NimBLEDevice::setMTU(128);
  NimBLEDevice::setPower(ESP_PWR_LVL_P9);
  NimBLEDevice::setSecurityAuth(false,false,false);

  auto server = NimBLEDevice::createServer();
  server->setCallbacks(new CbServer());

  auto svc = server->createService(SVC_UUID);
  imuChar = svc->createCharacteristic(
      IMU_UUID, NIMBLE_PROPERTY::NOTIFY
  );
  auto ctrl = svc->createCharacteristic(
      CTRL_UUID, NIMBLE_PROPERTY::WRITE
  );
  ctrl->setCallbacks(new CbCtrl());
  svc->start();

  auto adv = NimBLEDevice::getAdvertising();
  adv->addServiceUUID(SVC_UUID);
  
  adv->setMinInterval(32);
  adv->setMaxInterval(244);
  
  // Enhanced discoverability
  adv->setName("ServeSense");
  adv->setAppearance(0x0040); 
  
  adv->start();

  Serial.print("[BLE] Advertising as S with svc 0xFF00\r\n");
  Serial.print("========================================\r\n");
  Serial.print("[SETUP] Ready for connections!\r\n");
  Serial.print("========================================\r\n");
}

// ============================================================
// Main loop
// ============================================================
void loop() {
  static uint32_t last = 0;
  static bool lastSwitchState = false;

  // Check switch state for recording control
  bool switchOn = digitalRead(PIN_RECORD_SWITCH) == LOW;
  
  // Detect switch transitions
  if (switchOn && !lastSwitchState) {
    // Switch turned ON: start recording
    captureEnabled = true;
    sessionId++;
    sampleSerial = 0;
    serveMarker = true;
    digitalWrite(PIN_STATUS_LED, LOW); // Turn LED ON when recording
    Serial.print("[SWITCH] Recording STARTED (session ");
    Serial.print(sessionId);
    Serial.print(")\r\n");
  } else if (!switchOn && lastSwitchState) {
    // Switch turned OFF: stop recording
    captureEnabled = false;
    digitalWrite(PIN_STATUS_LED, HIGH); // Turn LED OFF when not recording
    Serial.print("[SWITCH] Recording STOPPED\r\n");
  }
  lastSwitchState = switchOn;

  // Only stream if enabled
  if (!streamOn) {
    delay(5);
    return;
  }

  uint32_t now = millis();
  if (now - last < PERIOD_MS) {
    delay(1);
    return;
  }
  last = now;

  float ax, ay, az, gx, gy, gz;
  if (!icmRead(ax, ay, az, gx, gy, gz)) {
    return;
  }

  // Only send data when recording is enabled
  if (!captureEnabled) {
    delay(1);
    return;
  }

  // Print CSV to monitor
  Serial.print("t=");
  Serial.print(now);
  Serial.print(",");
  Serial.print(sessionId);
  Serial.print(",");
  Serial.print(sampleSerial);
  Serial.print(",");
  Serial.print(ax, 4);
  Serial.print(",");
  Serial.print(ay, 4);
  Serial.print(",");
  Serial.print(az, 4);
  Serial.print(",");
  Serial.print(gx, 4);
  Serial.print(",");
  Serial.print(gy, 4);
  Serial.print(",");
  Serial.print(gz, 4);
  Serial.print(",1\r\n");

  // Check if we have a connection before sending
  if (!NimBLEDevice::getServer()->getConnectedCount()) {
    sampleSerial++;
    delay(1);
    return;
  }

  // Build and send ServeSense packet
  ServePacket pkt{};
  pkt.millis_ms = now;
  pkt.session   = sessionId;
  pkt.sequence  = sampleSerial++;
  pkt.ax = ax; pkt.ay = ay; pkt.az = az;
  pkt.gx = gx; pkt.gy = gy; pkt.gz = gz;
  pkt.flags = 0x01 | (serveMarker ? 0x02 : 0x00);

  serveMarker = false;

  try {
    imuChar->setValue(reinterpret_cast<uint8_t*>(&pkt), sizeof(pkt));
    imuChar->notify();
  } catch (...) {
    Serial.print("[ERR] BLE notification failed\r\n");
  }
}

